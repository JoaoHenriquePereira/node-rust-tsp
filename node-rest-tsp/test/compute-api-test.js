...
describe('compute', function() {
  	...
  	it('Must filter input for bad requests', function(done) {
  		...
  		let entry_point = '/'+pjson.name+'/compute'

  		let expected_json_schema = require('../schemas/compute-api-schema-output.json');

  		let bad_request_json = {
	...
	it('Must allow access to result', function(done) {
		...
		let expected_compute_json_schema = require('../schemas/compute-api-schema-output.json');
  		let expected_result_json_schema = require('../schemas/result-api-schema-output.json');
		let compute_entry_point = '/'+pjson.name+'/compute';
		let result_entry_point = '/'+pjson.name+'/result';

		let good_request_json = {
  			"graph_type": "u2d-cartesian",	//Unsigned 2d-cartesian coordinates
  			"graph": {
  				"nodes": [
  					{ "name": "1", "coordinates": [ 41.1621, 8.6220 ] },
